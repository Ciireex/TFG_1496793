...
from gym_strategy.envs.StrategyEnvRecurrent import StrategyEnvRecurrent
...
def make_env_blue():
    env = StrategyEnvRecurrent(blue_team=blue_team, red_team=red_team)
...
# Crear modelos recurrentes
model_blue = RecurrentPPO(
    ...
    batch_size=256,
    clip_range=0.2,
    tensorboard_log="./tensorboard/recurrent/",
    policy_kwargs=dict(net_arch=[dict(pi=[128], vf=[128])])
)

model_red = RecurrentPPO(
    policy="MlpLstmPolicy",
    env=make_env_red(),
    verbose=1,
    learning_rate=1e-4,
...
for i in range(10):
    ...
    model_blue.save(os.path.join(output_dir, f"recurrent_BLUE_ciclo{i+1}"))

    print(f"\nCiclo {i+1} - Entrenando equipo ROJO")
    model_red.learn(total_timesteps=100_000, callback=LogCallback())
    model_red.save(os.path.join(output_dir, f"recurrent_RED_ciclo{i+1}"))

print("Entrenamiento recurrente finalizado.")
