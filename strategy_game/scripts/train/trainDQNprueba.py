...
# Wrapper para registrar el equipo controlado
class DualTeamEnvWrapper(gym.Wrapper):
    def __init__(self, base_env, controlled_team):
        super().__init__(FlattenObservation(base_env))  # ✅ aplanar observación aquí
...
# Configuración
exp_name = "dqn_ppov10"
blue_path = f"models/dqn_blue_{exp_name}.zip"
red_path = f"models/dqn_red_{exp_name}.zip"
...
blue_model = DQN(
    ...
    verbose=1,
    tensorboard_log=f"logs/dqn_blue_{exp_name}",
    learning_rate=1e-4,
    buffer_size=100_000,
...
red_model = DQN(
    ...
    verbose=1,
    tensorboard_log=f"logs/dqn_red_{exp_name}",
    learning_rate=1e-4,
    buffer_size=100_000,
    learning_starts=1000,
    batch_size=64,
...
for cycle in range(start_cycle, end_cycle + 1):
    ...
    shutil.copy(blue_path, os.path.join(backup_dir, f"dqn_blue_{exp_name}_cycle{cycle}.zip"))
    ...
    shutil.copy(red_path, os.path.join(backup_dir, f"dqn_red_{exp_name}_cycle{cycle}.zip"))
