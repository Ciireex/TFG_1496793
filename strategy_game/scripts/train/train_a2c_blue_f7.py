...
LOG_DIR = os.path.abspath(os.path.join(BASE_DIR, "../../logs/a2c/blue_f7_v1"))
...
# === WRAPPER: RED congelado con acción válida aleatoria
...
# === CREACIÓN DEL MODELO AZUL F7 CON TRANSFER ===
model = A2C(
    policy="CnnPolicy",
    env=env,
...
print("🔁 Transferencia real desde F6 (cargando solo pesos de la política)...")
...
callbacks = [
    ...
    CheckpointCallback(
        save_freq=50000,
        save_path=MODEL_DIR,
        name_prefix="a2c_blue_f7_v1"
    )
]

# === ENTRENAMIENTO ===
print("🚀 Entrenando A2C azul F7 v1 (transfer desde F6)...")
model.learn(total_timesteps=1_000_000, callback=callbacks, progress_bar=True)

# === GUARDADO FINAL ===
model.save(os.path.join(MODEL_DIR, "a2c_blue_f7_v1"))
print("✅ Modelo azul F7 v1 guardado.")
