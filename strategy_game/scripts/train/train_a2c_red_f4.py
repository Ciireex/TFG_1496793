...
LOG_DIR = os.path.abspath(os.path.join(BASE_DIR, "../../logs/a2c/red_f4_v1"))
MODEL_DIR = os.path.abspath(os.path.join(BASE_DIR, "../../models"))
PREVIOUS_MODEL_PATH = os.path.join(MODEL_DIR, "a2c_red_f3_v4.zip")  # ← Transferencia desde F3
BLUE_MODEL_PATH = os.path.join(MODEL_DIR, "a2c_blue_f4_v1.zip")

os.makedirs(LOG_DIR, exist_ok=True)
os.makedirs(MODEL_DIR, exist_ok=True)

# === WRAPPER: AZUL CON POLÍTICA FIJA ===
class FrozenBlueWrapper(Env_Fase4_Archer):
    def __init__(self):
        super().__init__()
...
# === MODELO A2C Y TRANSFER DESDE F3 ===
model = A2C(
    policy="CnnPolicy",
    env=env,
...
callbacks = [
    ...
    CheckpointCallback(
        save_freq=50000,
        save_path=MODEL_DIR,
        name_prefix="a2c_red_f4_v1"
    )
]

# === ENTRENAMIENTO ===
print("🚀 Fine-tuning A2C rojo F4 v1 desde F3 v4 (con entorno de arqueros)...")
model.learn(total_timesteps=1_000_000, callback=callbacks, progress_bar=True)

# === GUARDAR ===
model.save(os.path.join(MODEL_DIR, "a2c_red_f4_v1"))
print("✅ Modelo F4 v1 rojo guardado.")
