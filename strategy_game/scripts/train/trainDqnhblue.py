...
class DualTeamVsHeuristicWrapper(gym.Wrapper):
    def __init__(self, base_env, controlled_team):
        ...
        self.heuristic = None  # Se inicializa tras el primer reset
        self.episode_count = 0

    def reset(self, **kwargs):
        obs, info = self.env.reset(**kwargs)
        if self.heuristic is None:
            self.heuristic = HeuristicPolicy(self.env)  # El entorno ya ha sido reseteado
...
# Crear entorno con wrapper y flatten
...
# Configuración
model_path = "models/dqn_vs_heuristic_blue.zip"
os.makedirs("models", exist_ok=True)
os.makedirs("logs", exist_ok=True)

# Crear entorno y cargar o crear modelo
env = make_env_vs_heuristic(team=0)
...
if os.path.exists(model_path):
    ...
    model = DQN(
        ...
        tensorboard_log="logs/dqn_vs_heuristic_blue",
        device="cpu"
    )
...
# Evaluación
mean_rew = evaluate_agent(model, team=0)
