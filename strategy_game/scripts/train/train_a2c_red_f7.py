...
LOG_DIR = os.path.abspath(os.path.join(BASE_DIR, "../../logs/a2c/red_f7_v1"))
...
# === WRAPPER: Azul congelado con acción válida aleatoria
...
# === CREACIÓN DEL MODELO ROJO F7 CON TRANSFER ===
model = A2C(
    policy="CnnPolicy",
    env=env,
...
print("🔁 Cargando pesos del modelo rojo F6 (transfer learning real)...")
...
callbacks = [
    ...
    CheckpointCallback(
        save_freq=50000,
        save_path=MODEL_DIR,
        name_prefix="a2c_red_f7_v1"
    )
]

# === ENTRENAMIENTO ===
print("🚀 Entrenando A2C rojo F7 v1 (contra azul F7 congelado)...")
model.learn(total_timesteps=1_000_000, callback=callbacks, progress_bar=True)

# === GUARDADO FINAL ===
model.save(os.path.join(MODEL_DIR, "a2c_red_f7_v1"))
print("✅ Modelo rojo F7 v1 guardado.")
