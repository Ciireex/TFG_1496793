...
print("🏁 Entrenando sin obstáculos (MaskablePPO rojo vs heurística azul)...")
env_no_obs = DummyVecEnv([
    lambda: ActionMasker(
        DualTeamEnvWrapper(StrategyEnv(use_obstacles=False), controlled_team=1),
        mask_fn
    )
])
model = MaskablePPO(
    ...
    env_no_obs,
    verbose=1,
    tensorboard_log="./logs/ppored_vs_heuristic_curriculum_maskable/",
    device="cpu"
)
model.learn(total_timesteps=1_000_000)

# === ENTRENAMIENTO FASE 2: con obstáculos ===
print("🏁 Entrenando con obstáculos (MaskablePPO rojo vs heurística azul)...")
...
# === GUARDAR MODELO ===
model.save("models/ppored_vs_heuristic_curriculum_maskable_v1")
print("\n✅ ENTRENAMIENTO COMPLETADO CON MaskablePPO (ROJO) VS HEURÍSTICA ✅")
