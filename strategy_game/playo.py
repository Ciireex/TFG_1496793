...
blue_model = A2C.load("models/a2c_blue_sparte_minimal_v7.zip")
red_model = A2C.load("models/a2c_red_sparte_minimal_v7.zip")

# Inicializar entorno y renderer
env = StrategyEnvSparteMinimal()
...
while not terminated and not truncated:
    print(f"\nTurno del equipo {'AZUL' if env.current_player == 0 else 'ROJO'} - Fase: {env.phase.upper()}")
