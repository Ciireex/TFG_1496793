...
class StrategyEnv_CNN(gym.Env):
    ...
    def step(self, action):
        ...
        if action == 0 and self.phase == "attack":
            px, py = unit.position
            if self._valid_coord((px, py)) and self._get_obs()[14, px, py] > 0.5:
                reward -= 0.2
        ...
        if hasattr(self, '_terminated') and self._terminated:
            ...
            if hasattr(self, '_winner'):
                if self._winner == self.current_player:
                    reward += self._last_reward  # gané
                else:
                    reward -= 2.0  # perdí
                del self._winner
            else:
                reward += self._last_reward  # empate por turnos
        ...
        if idx < len(my_units):
            ...
            for dx, dy in dxs:
                for dist in range(1, 4 if my_units[idx].unit_type == "Archer" else 2):
                    ...
                    for enemy in self.units:
                        if enemy.team != self.current_player and enemy.is_alive() and enemy.position == (tx, ty):
                            obs[14, tx, ty] = 1.0  # cambia a la casilla objetivo
