...
class StrategyEnvDuel(gym.Env):
    ...
    def reset(self, *, seed=None, options=None):
        ...
        self.units = [
            Soldier(position=self.spawn1, team=0),
            Soldier(position=self.spawn2, team=1)
        ]
    ...
    def step(self, action):
        ...
        if action <= 4:
            ...
            if other.is_alive() and other.position == target_pos:
                other.health -= 34  # 🔵 Nuevo daño: 34 de vida
                reward += 0.1  # Premio por acertar ataque
            else:
                reward -= 0.05  # Penalización por atacar al aire

        terminated = False

        # Eliminar enemigo
        if not other.is_alive():
            reward = 1.0  # ⚔️ Si matas, ganas
            terminated = True

        # Captura del punto
        if me.position == self.capture_point:
            if self.capture_progress[self.current_player] > 0 or self.capture_progress[1 - self.current_player] == 0:
                reward += 0.3  # 🔥 Subido premio por capturar
            self.capture_progress[self.current_player] += 1
            if self.capture_progress[self.current_player] >= self.capture_turns_required:
                reward = 1.5  # 🎯 Más premio si capturas
                terminated = True
        else:
            if self.capture_progress[self.current_player] > 0:
                reward -= 0.2  # ❌ Penalización por abandonar la captura
