...
class StrategyEnv_Fase2(gym.Env):
    ...
    def reset(self, *, seed=None, options=None):
        ...
        blue_types = [Soldier, Soldier, Archer, Archer]
        red_types = [Soldier, Soldier, Archer, Archer]
        random.shuffle(blue_types)
        random.shuffle(red_types)

        for i, unit_class in enumerate(blue_types):
            self.units.append(unit_class(position=blue_positions[i], team=0))
        for i, unit_class in enumerate(red_types):
        ...
        if self.use_obstacles:
            ...
            for col in range(self.board_size[1]):
                self.obstacles[reserved_path_row, col] = 0
            num_obstacles = 12
            attempts = 0
            while num_obstacles > 0 and attempts < 100:
                x = np.random.randint(1, self.board_size[0] - 1)
                y = np.random.randint(0, self.board_size[1])
                if x == reserved_path_row:
                    attempts += 1
                    continue
                if (x, y) in blue_positions or (x, y) in red_positions:
                    attempts += 1
                    continue
