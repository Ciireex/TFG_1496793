...
class StrategyEnvSparteMinimal(gym.Env):
    def __init__(self):
        super().__init__()
        self.board_size = (5, 5)
        self.capture_point = (2, 2)
        self.max_turns = 60
        self.capture_turns_required = 3

        self.unit_types = [Soldier, Soldier, Archer, Soldier, Soldier, Archer]
        self.num_units = 6

        self.action_space = spaces.Discrete(5)
        self.observation_space = spaces.Box(0, 1, shape=(13, 5, 5), dtype=np.float32)
    ...
    def reset(self, *, seed=None, options=None):
        ...
        self.attacked_unit_on_point = [False, False]
        self.units = []

        blue_spawns = [(0, 1), (0, 2), (0, 3)]
        red_spawns = [(4, 1), (4, 2), (4, 3)]

        for i in range(6):
            team = 0 if i < 3 else 1
            pos = blue_spawns[i] if team == 0 else red_spawns[i - 3]
    ...
    def step(self, action):
            ...
            for enemy in self.units:
                if enemy.team != self.current_player and enemy.is_alive() and enemy.position == (tx, ty):
                    ...
                    if enemy.position == self.capture_point:
                        self.attacked_unit_on_point[enemy.team] = True
        ...
        if self.phase == "move" and self.active_unit_index == 0:
            ...
            team_on_point = any(
                u.position == self.capture_point and u.team == team and u.is_alive()
                for u in self.units
            )

            if team_on_point:
                if self.attacked_unit_on_point[team]:
                    self.capture_progress[team] = 0
                    self.attacked_unit_on_point[team] = False
                else:
        ...
        if not any(u.is_alive() and u.team != self.current_player for u in self.units):
            reward += 3.0
            terminated = True
    ...
    def _get_obs(self):
        obs = np.zeros((13, self.board_size[0], self.board_size[1]), dtype=np.float32)
        ...
        for unit in self.units:
            if unit.is_alive():
                ...
                obs[idx, x, y] = 1.0
                obs[type_idx, x, y] = 1.0 if unit.unit_type == "Archer" else 0.5
