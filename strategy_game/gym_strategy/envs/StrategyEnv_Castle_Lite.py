...
class StrategyEnv_Castle_Lite(gym.Env):
    def __init__(self, use_obstacles=True, obstacle_count=6):
        super().__init__()
        self.board_size = (10, 6)
        self.castle_area = [(4, 2), (4, 3), (5, 2), (5, 3)]
        self.max_turns = 60
        self.castle_control = 0
        self.unit_types = [Soldier, Soldier, Soldier, Archer, Archer] * 2
        self.num_units = 10
    ...
    def reset(self, *, seed=None, options=None):
        ...
        blue_positions = [(0, y) for y in range(1, 5)] + [(1, y) for y in range(1, 5)]
        red_positions = [(9, y) for y in range(1, 5)] + [(8, y) for y in range(1, 5)]
        random.shuffle(blue_positions)
        random.shuffle(red_positions)

        for i, unit_class in enumerate(self.unit_types[:5]):
