...
class StrategyEnv_TransferSmall_1v1(gym.Env):
    def __init__(self):
        super().__init__()
        self.board_size = (6, 4)
        self.max_turns = 40
        self.unit_types = [Soldier, Soldier] * 2
        self.num_units = 4
        self.castle_area = []
    ...
    def reset(self, *, seed=None, options=None):
        ...
        blue_positions = [(0, 1), (0, 2)]
        red_positions = [(5, 1), (5, 2)]

        for i, unit_class in enumerate(self.unit_types[:2]):
    ...
    def step(self, action):
        ...
        dx, dy = dirs[action]
        attacked = False
        enemy_in_range = False  # ← Nueva bandera
        ...
        for dist in range(min_range, unit.attack_range + 1):
            ...
            for enemy in self.units:
                if enemy.team != self.current_player and enemy.is_alive() and enemy.position == (tx, ty):
                    enemy_in_range = True  # ← Enemigo en rango
        ...
        # Penalizar si había enemigo en rango y no lo atacó
        if enemy_in_range and not attacked:
            reward -= 0.3  # ⚠️ Puedes ajustar este valor

        self._advance_phase()

        if self.turn_count >= self.max_turns:
            reward -= 2.0  # penalización fuerte por empate
            terminated = True
