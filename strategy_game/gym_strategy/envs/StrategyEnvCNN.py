...
class StrategyEnvCNN(gym.Env):
    def __init__(self):
        ...
        self.renderer = Renderer(board_size=self.board_size)
        self.reset()
    ...
    def _get_obs(self):
        ...
        return cv2.resize(img, (84, 84), interpolation=cv2.INTER_AREA).astype(np.uint8)

    def _render_screen(self):
        self.renderer.draw_board(
            units=self.units,
            blocked_positions=[(x, y) for x in range(self.board_size[0]) for y in range(self.board_size[1]) if self.obstacles[x, y] == 1],
            capture_point=self.capture_point,
            capture_progress=self.capture_progress,
            capture_max=self.capture_turns_required,
            capturing_team=self.current_player,
            active_unit=self.units[self.active_unit_index] if self.active_unit_index < len(self.units) else None
        )
